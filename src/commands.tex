...
idioma previamente carregado nas opções da classe (\cref{sec:hifenizacao}). Nesse
...
o expoente, sem espaço entre elas e com fonte menor~\citeonline[5.2.1]{NBR14724:2011}.}\footnote{Caso uma série de notas sejam
...
\section{Tabelas}%
\index{tabelas}A~\cref{tab:nivinv} é um exemplo de tabela construída em
\LaTeX.
...
Já a~\cref{tab:ibge} apresenta uma tabela criada conforme o padrão do~\citeonline{ibge1993} requerido pelas normas da ABNT para documentos técnicos e
acadêmicos.
...
\cref{fig:grafico}. Se a figura que for incluída se tratar de um diagrama, um
...
\index{CorelDraw} ou ao Adobe
Illustrator%
\index{Adobe Illustrator}. De todo modo, caso não seja possível
...
\cref{fig:minipage_imagem1} e da~\cref{fig:minipage_grafico2}.
...
\index{alíneas}%
\index{subalíneas}%
\index{incisos}Quando for necessário enumerar
...
\cref{ch:exemplos}, que tem o nome \emph{\nameref{ch:exemplos}}. O número
...
\cref{sec:divisoes}. Veja a~\cref{subsec:divisoes}.

\subsection{Divisões do documento: subseção}%
\label{subsec:divisoes}

Isto é uma subseção. Veja a~\cref{subsubsec:divisoes}, que é uma
...
\subsubsubsection{Esta é outra subseção de quinto nível}%
...
dos principais objetivos do \abnTeX. Consulte os manuais~\citeonline{abntex2cite} e~\citeonline{abntex2cite-alf} para obter informações
